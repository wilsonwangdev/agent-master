...
示例：本项目中的 `skills/build-code-review.md` 检查：
...
当前：在 AGENTS.md 中说明修改某些文件后应运行该 skill。Agent 读取这条指令并遵循。
...
Claude Code 在多个时点支持 hooks：
- `PostToolUse` — 在 Edit 或 Write 工具调用之后
- `PostCommit` — 在 git commit 之后

对 `build/*` 文件的 PostToolUse hook 可以在每次编辑构建系统后自动运行 build code review skill。
